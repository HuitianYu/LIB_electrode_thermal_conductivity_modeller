%% K_MODEL_AUTOGENERATOR This function automatically creates k_model from k_model_calculator.

function k_model_autoGenerator(modelName_core,selectiveParameters)
...
%% Creation of the k_model m-file
	...
	eval(['fid = fopen(''k_',modelName_core,'.m'',''w'');'])
    ...
    str = ['%%%% K_',upper(modelName_core),' This model calculates the effective thermal conductivity using names from table statistics\n'];
    ...
    str = ['function keff = k_',modelName_core,'(namesMatrix,volumeFractionsMatrix,nameFiller,porosity'];
    strlength_str = strlength(str);
    ...
    strlength_str = strlength(str);
    str = insertAfter(str,strlength_str,['\t','%% namesMatrix: e.g. {''PVDF'';''carbonBlack''} or {''PVDF''}: n_matrix * 1 numeric array of thermal conductivities.\n']);
    strlength_str = strlength(str);
    str = insertAfter(str,strlength_str,['\t','%% volumeFractionsMatrix:   e.g. [0.5] or []: (n_matrix - 1) * 1 numeric array. The sum should not exceed 1. The last one can be omitted.\n']);
    strlength_str = strlength(str);
    str = insertAfter(str,strlength_str,['\t','%% thermalConductivityFiller: e.g. ''none'' or ''carbonBlack'': numeric variable.\n']);    
    strlength_str = strlength(str);
    str = insertAfter(str,strlength_str,['\t','%% porosity: e.g. 0 or 0.5: numeric variable.\n']);
    ...
    strlength_str = strlength(str);
    str = insertAfter(str,strlength_str,['\t','keff = k_model_out_of_name(namesMatrix,volumeFractionsMatrix,nameFiller,porosity,modelNamesParameters);']);
    % write to file and delete str
    fprintf(fid,str);
    clear str strlength_str
