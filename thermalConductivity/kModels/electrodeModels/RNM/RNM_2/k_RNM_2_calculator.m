%% K_RNM_2_CALCULATOR This function calculates the effective thermal conductivity of active material, conductive additve, binder filled with filler.
% part 1:   only filler, part 2: only active material,
% part 3:   parallel model for additive (conductive additive + binder) and filler,
%           which is series connected with active material

function keff = k_RNM_2_calculator(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,modeTortuosity,a)
...
%% Calculation of effective thermal conductivity
    ...
    vFrSolid_binder = 1 - vFrMatrix_activeMaterial - vFrMatrix_conductiveAdditive;

    % volume fractions of conductive additive and binder in additive, summing up to 1
    if vFrMatrix_conductiveAdditive + vFrSolid_binder ~= 0
        vFrAdditive_conductiveAdditive = vFrMatrix_conductiveAdditive / (vFrMatrix_conductiveAdditive + vFrSolid_binder);
        vFrAdditive_binder = vFrSolid_binder / (vFrMatrix_conductiveAdditive + vFrSolid_binder);
    ...
    % volume fractions in parallel system: 
        ...
        % part 3
        vFr_3_activeMaterial = ( 1 - a ) * vFrMatrix_activeMaterial * (1 - porosity); % active part
        
        vFr_3_conductiveAdditive = vFrMatrix_conductiveAdditive * (1 - porosity); % non-active part
        vFr_3_binder = vFrSolid_binder * ( 1- porosity);        
        vFr_3_filler = porosity - porosity / tortuosity;
        vFr_3_nonActiveMaterial = vFr_3_conductiveAdditive + vFr_3_binder + vFr_3_filler; % NA: non-active
        
        vFr_3 = vFr_3_activeMaterial + vFr_3_nonActiveMaterial;
    ...
    if vFr_3 ~= 0
        ...
        vFrPart3_nonActiveMaterial = vFr_3_nonActiveMaterial / vFr_3;

        % check whether the sum of all volume fractions is 1
        vFrPart3_total = vFrPart3_activeMaterial + vFrPart3_nonActiveMaterial;
        if abs(vFrPart3_total - 1) > esp
            error('The sum of volume fractions should be 1.')
        end
    else
        vFrPart3_activeMaterial = 0;
        vFrPart3_nonActiveMaterial = 0;
    ...
    % volume fractions in non-active materials
    if vFr_3_nonActiveMaterial ~= 0
        vFrPart3NonActiveMaterial_conductiveAdditive = vFr_3_conductiveAdditive / vFr_3_nonActiveMaterial;
        vFrPart3NonActiveMaterial_binder = vFr_3_binder / vFr_3_nonActiveMaterial;
        vFrPart3NonActiveMaterial_filler = vFr_3_filler / vFr_3_nonActiveMaterial;

        % check whether the sum of all volume fractions is 1
        vFrPart3NonActiveMaterial_total = vFrPart3NonActiveMaterial_conductiveAdditive + vFrPart3NonActiveMaterial_binder + vFrPart3NonActiveMaterial_filler;
        if abs(vFrPart3NonActiveMaterial_total - 1) > esp
            error('The sum of volume fractions should be 1.')
        end
    else
        vFrPart3NonActiveMaterial_conductiveAdditive = 0;
        vFrPart3NonActiveMaterial_binder = 0;
        vFrPart3NonActiveMaterial_filler = 0;

        % check whether the sum of all volume fractions is 0
        vFrPart3NonActiveMaterial_total = vFrPart3NonActiveMaterial_conductiveAdditive + vFrPart3NonActiveMaterial_binder + vFrPart3NonActiveMaterial_filler;
        if abs(vFrPart3NonActiveMaterial_total) > esp
            error('The sum of volume fractions should be 0.')
        end        
    end
    ...
    k_part3_nonActiveMaterial = k_conductiveAdditive * vFrPart3NonActiveMaterial_conductiveAdditive + k_binder * vFrPart3NonActiveMaterial_binder + k_filler * vFrPart3NonActiveMaterial_filler;
    k_3 = 1 / (vFrPart3_activeMaterial / k_activeMaterial + vFrPart3_nonActiveMaterial / k_part3_nonActiveMaterial);
