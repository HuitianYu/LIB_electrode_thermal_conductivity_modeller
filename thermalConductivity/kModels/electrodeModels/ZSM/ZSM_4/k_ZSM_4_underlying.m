%% K_ZSM_4_UNDERLYING This function calculates the effective thermal conductivity of active material, conductive additve, binder filled with filler.
...
function [deviation_vFr_matrixPassage,keff] = k_ZSM_4_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,allPorosity,curveName,modelAdditiveName,modeTortuosity,a,c,e,g)
%% Explanations of input & output arguments:
    ...
    % e: e.g. 0.1: numeric variable in range of [0,1], disperse level in part 3.
...
%% Calculation of effective thermal conductivity
    ...
    vFr_3_filler = porosity - vFr_1 + vFr_filler_in_additive_2;
    if vFr_3_filler < 0 || vFr_3_filler > porosity
        error('vFr_3_filler is negative or bigger than the whole porosity')
    end
    vFrDispersed_additive_2_real = vFr_additive_2_real / (vFr_additive_2_real + vFr_3_filler);
    % calculating the thermal conductivity of the homogeneously dispersed additive system in filler (fluid) using EMT model
    k_dispersed = k_model_out_of_name_calculator([k_additive_pure;k_filler],vFrDispersed_additive_2_real,'none',0,'EMT,f=6');
    ...
    k_3_down = RAdditive_1_part3^2 * k_additive + (1 - RAdditive_1_part3^2) * k_dispersed;
