%% K_ZSM_2_ADDINFILLER_CALCULATOR This function calculates the effective thermal conductivity of active material, conductive additve, binder filled with filler.
...
function keff = k_ZSM_2_addInFiller_calculator(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,curveName,modelAdditiveName,modeTortuosity,alpha,c,e,g,modelAddInFillerName)
...
%% Calculation of effective thermal conductivity
    ...
    if ischar(c) && strcmp(c,'automatic')
        ...
        if porosity == 1
            ...
            Func_deviation_vFr_matrixPassage = @(c) k_ZSM_2_addInFIller_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,curveName,modelAdditiveName,modeTortuosity,a,c,e,g);
            c_left = 0;
            if abs(Func_deviation_vFr_matrixPassage(c_left)) < deviationEsp
                [deviation,keff] = k_ZSM_2_addInFiller_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,...
    ...
    elseif isnumeric(c)
        c_res = c;
        [~,keff] = k_ZSM_2_addInFiller_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,curveName,modelAdditiveName,modeTortuosity,alpha,c_res,e,g,modelAddInFillerName);
    else
