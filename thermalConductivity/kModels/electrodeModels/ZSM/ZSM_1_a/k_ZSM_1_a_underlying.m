%% K_ZSM_1_A_UNDERLYING This function calculates the effective thermal conductivity of active material, conductive additve, binder filled with filler.
% part 1: only filler (up-part & down-part), part 2: active material
% part 3:   point contact Zehner-Schlunder model with activematerial, additives(1 and 2) and filler as up-part;
...
function [deviation_vFr_matrixPassage,keff] = k_ZSM_1_a_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,allPorosity,curveName,modelAdditiveName,modeTortuosity,a,c,e,g)
...
%% Calculation of effective thermal conductivity
    ...
    k_additive = k_model_out_of_name_calculator([k_additive_pure;k_filler],1 - innerPorosity_in_additive,'none',0,'EMT,f=6');
    ...
    vFr_2_up = vFrMatrix_activeMaterial * (1 - porosity) * a * (1-c); % only active material
    if a == 0
        vFr_2_down = 0;
        vFr_2 = 0;
    else
    ...
    if e == 1
        ...
        f_RAdditive_1_part3 = @(RAdditive_1_part3) ( Func_VAdditive_1(RAdditive_1_part3) / (pi / 4) / (1 - e) * (1 - c)  + RAdditive_1_part3^2 * c ) * R3^2 - vFr_additive;
    ...
    if e == 0
        ...
        if e == 1
            ...
            vFr_additive_2 = vFr_additive - ( Func_VAdditive_1(RAdditive_1_part3) / (pi / 4) * (1 - c)  + RAdditive_1_part3^2 * c ) * R3^2;
        end
        if vFr_additive_2 < 0 
            error('vFr_additive_2 is negative.')
        end
        eval(['Func_VAdditive_2 = @(RAdditive_2_part3) volume8th_additive_2(@(r,B,alpha)',curveName,'(r,B0,0),RAdditive_2_part3);'])
        f_RAdditive_2_part3 = @(RAdditive_2_part3) Func_VAdditive_2(RAdditive_2_part3) / (pi / 4) * (1 - c) * R3^2 - vFr_additive_2;

        % calculate RAdditive_2_part3 using fsolve iteratively
        options = optimoptions('fsolve','Display','none','TolFun',1e-20);
        RAdditive_2_part3_iter = fsolve(f_RAdditive_2_part3,R3/2,options);

        % check whether RAdditive_2_part3_iter is valid
        if abs(f_RAdditive_2_part3(RAdditive_2_part3_iter))/ vFr_additive_2 > VEsp
            error('Failed to find an accurate RAdditive_2_part3 (RAdditive_2_part3_iter = %.4f) for additive because of the inaccuracy (value: %.4f) of the calculated VAdd using this RAdditive_2_part3_iter (value: %.4f) at RAdditive_2_part3_iter = % .4f. \n',RAdditive_2_part3_iter,abs(f_RAdditive_2_part3(RAdditive_2_part3_iter)) / vFr_additive_2,RAdditive_2_part3_iter);
        else
            RAdditive_2_part3 = RAdditive_2_part3_iter;
        end
    end
    ...
    k_3_up = k_ZSM_additives_calculator(k_activeMaterial,k_filler,k_additive,curveName,1,B0,0,RAdditive_1_part3,RAdditive_2_part3);
    if c == 0
        k_3 = k_3_up;
    else
