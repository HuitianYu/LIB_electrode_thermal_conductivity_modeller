%% K_ZSM_1_A_POLYAM This model calculates the effective thermal conductivity using names from table statistics
% according to ZSM_1_a_polyAM.

function keff = k_ZSM_1_a_polyAM(namesMatrix,volumeFractionsMatrix,nameFiller,porosity,curveName,modelAdditiveName,modeTortuosity,a,c,e,g,volumeFractionsAM)
...
%% Calculation of the effective thermal conductivity
	...
	modelNamesParameters ='ZSM_1_a_polyAM';
	...
	strlength_modelNamesParameters = strlength(modelNamesParameters);
	modelNamesParameters = insertAfter(modelNamesParameters,strlength_modelNamesParameters,[',',getVarName(volumeFractionsAM),'=']);
	if ischar(volumeFractionsAM)
		strlength_modelNamesParameters = strlength(modelNamesParameters);
		modelNamesParameters = insertAfter(modelNamesParameters,strlength_modelNamesParameters,['''',volumeFractionsAM,'''']);
	elseif isnumeric(volumeFractionsAM)
		strlength_modelNamesParameters = strlength(modelNamesParameters);
		modelNamesParameters = insertAfter(modelNamesParameters,strlength_modelNamesParameters,'[');
		for ii = 1 : size(volumeFractionsAM,1)
			strlength_modelNamesParameters = strlength(modelNamesParameters);
			modelNamesParameters = insertAfter(modelNamesParameters,strlength_modelNamesParameters,[num2str(volumeFractionsAM(ii,1)),';']);
		end
		clear ii
		strlength_modelNamesParameters = strlength(modelNamesParameters);
		modelNamesParameters = insertAfter(modelNamesParameters,strlength_modelNamesParameters,']');
