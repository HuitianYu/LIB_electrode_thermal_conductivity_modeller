%% K_ZSM_5_ADDINFILLER_UNDERLYING This function calculates the effective thermal conductivity of active material, conductive additve, binder filled with filler.
...
function [deviation_vFr_matrixPassage,keff] = k_ZSM_5_addInFiller_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,allPorosity,curveName,modelAdditiveName,modeTortuosity,a,c,modelAddInFiller)
...
%% Calculation of effective thermal conductivity
    ...
    vFr_additive_real = (vFr_additive_2 + vFr_2_down) * (1-innerPorosity_in_additive);   % volume fraction of the additive without inner porosity
    vFr_filler_in_additive = (vFr_additive_2 + vFr_2_down) * innerPorosity_in_additive;  % volume fraction of filler in the inner porosity in additive 2
    vFr_filler = porosity - vFr_1 + vFr_filler_in_additive;
    if vFr_filler < 0 || vFr_filler > porosity
        error('vFr_filler is negative or bigger than the whole porosity')
    end
    vFrDispersed_additive_real = vFr_additive_real / (vFr_additive_real + vFr_filler);
    % calculating the thermal conductivity of the homogeneously dispersed additive system in filler (fluid) using EMT model
    k_dispersed = k_model_out_of_name_calculator([k_additive_pure;k_filler],vFrDispersed_additive_real,'none',0,modelAddInFiller);
    ...
    if c == 0
        ...
        k_2 = 1 / ( c / k_dispersed + (1 - c) / k_activeMaterial);
    end
    k_3_down = k_dispersed;
    k_3_up = k_ZSM_additive_calculator(k_activeMaterial,k_dispersed,k_dispersed,curveName,1,B0,0,RAdditive_1_part3);
    if c == 0
        k_3 = k_3_up;
    else
