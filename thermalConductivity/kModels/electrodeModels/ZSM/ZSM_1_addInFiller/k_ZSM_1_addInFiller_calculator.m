%% K_ZSM_1_ADDINFILLER_CALCULATOR This function calculates the effective thermal conductivity of active material, conductive additve, binder filled with filler.
...
function keff = k_ZSM_1_addInFiller_calculator(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,curveName,modelAdditiveName,modeTortuosity,a,c,e,g,modelAddInFillerName)
...
%% Calculation of effective thermal conductivity
    ...
    if ischar(c) && strcmp(c,'automatic')
       ...
       % set the maximal difference
        ...
        if porosity == 1
            ...
            Func_deviation_vFr_matrixPassage = @(c) k_ZSM_1_addInFiller_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,curveName,modelAdditiveName,modeTortuosity,a,c,e,g);
            c_left = 0;
            % fprintf('c_left = %6.4f.\n',c_left)
            if abs(Func_deviation_vFr_matrixPassage(c_left)) < deviationEsp
                [deviation,keff] = k_ZSM_1_addInFiller_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,...
                ...
                % fprintf('c_left = %6.4f, c_middle = %6.4f, c_right = %6.4f.\n',c_left,c_middle,c_right)
                try
                    [deviation,~] = Func_deviation_vFr_matrixPassage(c_middle);
                    cExist = true;
                    % fprintf('  Value at c_middle = %6.4f exists with deviation = %6.4f with c_right - c_left = %6.4f.\n',c_middle,deviationc_right - c_left)
                catch
                    cExist = false;
                    % fprintf('  Value at c_middle = %6.4f does NOT exist with c_right - c_left = %6.4f.\n',c_middle,c_right - c_left)
    ...
    elseif isnumeric(c)
        c_res = c;
        [~,keff] = k_ZSM_1_addInFiller_underlying(thermalConductivitiesMatrix,volumeFractionsMatrix,thermalConductivityFiller,porosity,curveName,modelAdditiveName,modeTortuosity,a,c_res,e,g,modelAddInFillerName);
    else
